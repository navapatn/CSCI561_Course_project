...
Original file is located at
    https://colab.research.google.com/drive/1cV5YrMUIsB9SK67xPg1r2uWWKZf7o6UF
"""
...
train_data_csv = pd.read_csv("train_image.csv",header=None)
test_data_csv =  pd.read_csv("test_image.csv",header=None)
#separating labels and pixels
train_labels_csv= pd.read_csv("train_label.csv",header=None)
test_labels_csv= pd.read_csv("test_label.csv", header=None)


test = pd.concat([test_labels_csv, test_data_csv], axis=1).reindex(test_labels_csv.index)


# merge labels with actual pixels data
#train_data = train_labels_csv.join(train_data_csv)
train_data = pd.concat([train_labels_csv, train_data_csv], axis=1).reindex(train_labels_csv.index)
#test_data = pd.concat([test_labels_csv, test_data_csv], axis=1).reindex(test_labels_csv.index)

train_data_merged = np.array(train_data)
test_data_merged = np.array(test_data_csv)
...
test_data_merged_sliced = test_data_merged
...
X_test = load_data_test(test_data_merged_sliced)

class NeuralNetwork:
    def __init__(self, X, y, batch = 16, lr = 1e-1,  epochs = 200):
      #### 60000
